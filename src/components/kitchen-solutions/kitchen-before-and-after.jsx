...
/**
 * Two kitchen transformations, compared.
 *
 * Unlike the shower and bathtub pages — whose "carousels" ran over a
 * transformations array of exactly one — this one really does have two pairs,
 * so the ability to switch between them is worth keeping. What it was wearing
 * was not: unlabelled dot buttons with no accessible name and no aria-current,
 * plus a pair of arrow buttons that were equally anonymous. A visitor using a
 * screen reader met four nameless buttons and no way to tell which pair was on
 * screen.
 *
 * It is a tablist now, named by the transformation titles that were already in
 * the content and previously shown only as a caption underneath.
 *
 * Both pairs are matched — 960x1280 and 1536x2048, both 3:4 portrait, same
 * framing — which is what the drag-to-reveal needs, so this uses the shared
 * slider. The key on it forces a remount when the pair changes, so the reveal
 * starts at the middle again rather than carrying the previous position over
 * onto a different photograph.
 *
 ...
 * were absolutely positioned inside frames that had no position of their own,
 * so they resolved against the outer card.
 */
        ...
        <div className="rule-hairline grid gap-10 border-t pt-10 lg:grid-cols-12 lg:gap-14">
          <div className="lg:col-span-6 lg:self-center">
            ...
            <Reveal delay={190} className="mt-8">
              <QuoteButton source="kitchen_before_after" label={labels.button} />
            </Reveal>
            ...
            <Reveal delay={230} className="mt-6">
              ...
              >
                ...
                <ArrowRight
                  aria-hidden="true"
                  className="animate-cta-arrow ml-2 h-4 w-4 shrink-0 transition-transform duration-200 ease-out group-hover:translate-x-1"
                />
              </Link>
            </Reveal>
          </div>
          ...
          <Reveal delay={160} className="lg:col-span-6">
            <div
              role="tablist"
              aria-label="Kitchen transformations"
            ...
            <div id="before-after-panel" className="mt-6">
              <BeforeAfterSlider
                ...
                labels={labels}
                subject="kitchen"
                className="mx-auto aspect-[3/4] max-w-[460px]"
