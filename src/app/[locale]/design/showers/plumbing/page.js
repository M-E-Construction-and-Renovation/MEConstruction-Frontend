...
function ShowerPlumbingConfiguration() {
  const router = useRouter();

  const searchParams = useSearchParams();

  const showerShape = searchParams.get("shape") || "unknown";

  const plumbingOptions = [
    {
      id: "left",
      label: "Left",
      image: "/images/shower-plumbing.png",
      href: `/design/showers/configure?plumbing=left&shape=${showerShape}`,
    },
    {
      id: "right",
      label: "Right",
      image: "/images/shower-plumbing-right.png",
      href: `/design/showers/configure?plumbing=right&shape=${showerShape}`,
    },
  ];
...
export default function ShowerPlumbingConfigurationWrapper() {
  return (
    <Suspense fallback={<div>Loading configurator...</div>}>
      <ShowerPlumbingConfiguration />
    </Suspense>
  );
}
